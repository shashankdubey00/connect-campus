...
const Chat = () => {
  ...
  const [collegeId, setCollegeId] = useState(null); // Added for literal Fix 1 adherence
...
const LiveChatView = ({ chat, college, onBack, onViewProfile, onViewStudentProfile, user, verificationStatus, onMessageSent }) => {
  ...
  const swipedMessageRef = useRef(null) // Track message being swiped for reply
  ...
  // Stable WhatsApp-like ordering (timestamp asc, tie-breaker by id)
  ...
  useEffect(() => {
    ...
    const handleReceiveMessage = (message) => {
      ...
      // FIX 4: Get current user ID from multiple sources
      ...
      if (message.collegeId === collegeId) {
        ...
        if (senderIdStr !== String(currentUserId) && blockedUsers.has(senderIdStr)) {
        ...
        // Prefer deterministic optimistic replacement if server echoed a clientMessageId
        setMessages(prev => {
          ...
          // Best-case correlation: replace optimistic message via clientMessageId (no clock skew / repeated text issues)
          if (message.clientMessageId) {
            const corrIndex = prev.findIndex(m =>
              m.isOptimistic && (m.id === message.clientMessageId || m.clientMessageId === message.clientMessageId)
            )

            if (corrIndex !== -1) {
              const optimisticMsg = prev[corrIndex]
              const newMessages = [...prev]
              ...
              console.log('✅ Replaced optimistic message via clientMessageId:', message.clientMessageId, '->', message.id)
              return newMessages
            }
          }
          ...
          if (optimisticIndex !== -1) {
            ...
            newMessages[optimisticIndex] = {
              id: message.id,
              clientMessageId: message.clientMessageId || optimisticMsg.clientMessageId || null,
              text: message.text,
              sender: message.senderName,
              senderId: message.senderId,
              time: new Date(message.timestamp).toLocaleTimeString('en-US', {
          ...
          const newMessages = [...filteredPrev, formattedMessage].sort(compareMessages)
  ...
  const handleSendMessage = async (e) => {
    ...
    // Create a stable client id so the server echo can deterministically replace this optimistic bubble
    const clientMessageId = `cmsg-${Date.now()}-${Math.random().toString(16).slice(2)}`
  ...
  // Long-press selection (mobile) + swipe-to-reply (mobile)
  ...
  const handleMessageTouchEnd = (e) => {
    ...
    // Commit swipe-to-reply if we were swiping
    if (isMobile && isSwipingRef.current && swipedMessageRef.current) {
      if (swipeOffset > 60) {
        setReplyingTo(swipedMessageRef.current)
        setTimeout(() => {
          const inputArea = document.querySelector('.chat-input-area')
          if (inputArea) inputArea.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
        }, 50)
      }
    }
  ...
  // Touch move: detect swipe-to-reply; otherwise cancel long-press during scroll
  const handleMessageTouchMove = (e) => {
    ...
    // Decide if this gesture is a horizontal swipe (prevents breaking vertical scroll)
    if (!isSwipingRef.current) {
      if (absX > 10 && absX > absY + 6) {
        isSwipingRef.current = true
        // Cancel long press once swipe begins
        if (longPressTimer.current) {
          clearTimeout(longPressTimer.current)
          longPressTimer.current = null
          longPressMessageId.current = null
        ...
        // Any small movement cancels long-press to avoid accidental selection while scrolling
        if (longPressTimer.current && (absX > 4 || absY > 4)) {
          clearTimeout(longPressTimer.current)
          longPressTimer.current = null
          longPressMessageId.current = null
        }
    ...
    // Active swipe: only allow right swipe
    if (dx > 0) {
      // Prevent vertical scroll while actively swiping horizontally (only if cancelable)
    ...
    const handleNewMessage = (data) => {
      ...
      if (data && data.groupId && String(data.groupId) === String(groupId)) {
        ...
        setMessages(prev => {
          ...
          return [...prev, newMessage].sort(compareMessages)
        })
